...
function tonav() {
  ...
  document.getElementById("page-name").innerText = document.title.split(" | Yuyu🍭")[0];
}
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by Yuyu🍭 %c 你正在访问Yuyu🍭の小家",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| Yuyu🍭") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('Yuyu🍭的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
...
if (m == 5 && dd == 20) {//小猫咪生日
  if (sessionStorage.getItem("isPopupWindow") != "1") {
    Swal.fire("祝小猫咪" + (y - 2006).toString() + "岁生日快乐！🐱");
    sessionStorage.setItem("isPopupWindow", "1");
  }
}
...
function createtime() {
  ...
  var start = new Date("06/19/2025 00:00:00"); // 旅行者1号开始计算的时间
  var dis = Math.trunc(23400000000 + ((now - start) / 1000) * 17); // 距离=秒数*速度 记住转换毫秒
  var unit = (dis / 149600000).toFixed(6);  // 天文单位
  // 网站诞生时间
  var grt = new Date("06/15/2025 00:00:00");
  ...
  (currentTimeHtml =
    hnum < 18 && hnum >= 9
      ...
      : `<img class='boardsign' src='/assets/F小屋-下班休息啦.svg' title='下班了就该开开心心地玩耍~'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`),
...
if (localStorage.getItem("blogbg") != undefined) {
  ...
  document.getElementById("defineBg").innerText = `:root{
    --default-bg: url(/assets/home_bg.jpg);
    --darkmode-bg:url(/assets/dark_mode.webp);
    --mobileday-bg: url(/assets/snow.webp);
    --mobilenight-bg: url(/assets/mb8.webp);
  }`;
}
// 切换背景主函数
function changeBg(s) {
